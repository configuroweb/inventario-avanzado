...
class Master extends DBConnection
{
	...
	function save_supplier()
	{
		...
		if ($save) {
			$resp['status'] = 'success';
			if (empty($id)) {
				$res['msg'] = "Proveedor guardado con éxito.";
				$id = $this->conn->insert_id;
			} else {
				$res['msg'] = "Proveedor actualizado con éxito.";
			}
	...
	function save_item()
	{
		...
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = "El producto ya existe en el proveedor seleccionado.";
			return json_encode($resp);
			exit;
		}
		if (empty($id)) {
	...
	function save_po()
	{
		...
		if ($resp['status'] == 'success') {
			if (empty($id)) {
				$this->settings->set_flashdata('success', " Nueva compra guardada correctamente.");
			} else {
				$this->settings->set_flashdata('success', " Orden de producto actualizada correctamente.");
			}
		}
	...
	function delete_po()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Detalles de orden eliminados con éxito.");
			if ($bo->num_rows > 0) {
	...
	function delete_receiving()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Detalles del pedido recibido Eliminados con éxito.");
	...
	function delete_return()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "El registro del artículo devuelto se eliminó con éxito.");
			if (isset($res)) {
				$this->conn->query("DELETE FROM `stock_list` where id in ({$res['stock_ids']})");
			}
		} else {
